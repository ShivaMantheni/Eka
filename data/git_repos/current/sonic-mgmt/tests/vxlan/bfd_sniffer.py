...
"""
from scapy.all import get_if_list, sniff, UDP, IP, IPv6
import sys
import json
try:
    ...
    for iface in ifaces:
        if iface.startswith("eth"):
            ...
            for pkt in output:
                if pkt.haslayer(UDP):
                    if pkt.haslayer(IP):
                        bfd_interfaces[pkt[IP].dst] = {
                            "src": pkt[IP].src, "iface": iface}
                    elif pkt.haslayer(IPv6):
                        bfd_interfaces[pkt[IPv6].dst] = {
                            "src": pkt[IPv6].src, "iface": iface}
