...
class CompletenessLevel(enum.IntEnum):
    ...
    def get_normalized_level(cls, request):
        ...
        For example, if a testcase supports "CompletenessLevel.basic, CompletenessLevel.thorough",
        and the specified level during test execution is "confident",
        then this method will normalize the level to "basic".
    ...
    def get_level_name(cls, level):
        ...
        if not isinstance(level, CompletenessLevel):
            logging.error("Invalid completeness type. Expected: {}. Format {}"
                          .format(str(CompletenessLevel), type(level)))
...
def set_default(specified_level):
    if not specified_level:     # Case 1
        logging.info("Completeness level not set during test execution. Setting to default level: {}"
                     .format(str(CompletenessLevel.basic)))
        ...
        if specified_level not in CompletenessLevel._member_names_:
            ...
            logging.info("Unidentified completeness level specified. Setting to default level: {}"
                         .format(CompletenessLevel.basic))
        else:
